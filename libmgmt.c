...
//user able to search books
//admin has to check the library_list for security

char password[8]="root@123"; //admin password
struct book
{
    int id;
    char name[20];
    char author[20];    //definition of structure of book
    int copies;
    int copiesleft;
}b,bc;
struct date
{
    int dd,mm,yy;      //definition of structure of date
};
struct user
{
    int id;
...
int main(void)
{
    system("cls");
    printf("******************************\n");
    printf("**WELCOME  TO OUR UNIVERSAL LIBRARY**\n");
    printf("one book is equal to 100 best friends\n");
    ...
    else
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    printf("\n\t The due date is %d-%d-%d",uc.due.dd,uc.due.mm,uc.due.yy);
                    printf("\n\t The fine to be collected is %d rupees",f);
                    getch();
                    break;
                }
                else if(f<0)
